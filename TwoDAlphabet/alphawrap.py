...
class Generic2D(object):
    ...
    def __init__(self,name,binning,forcePositive=True):
        ...
        self.binArgLists = {c:None for c in [str(c) for c in self.binning.xVars]}
        self.rph = {c:None for c in [str(c) for c in self.binning.xVars]}
    ...
    def _manipulate(self,name,other,operator=''):
        ...
        for cat in [str(c) for c in self.binning.xVars]:
